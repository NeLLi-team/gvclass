...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='Run HMM search and process the output')
    parser.add_argument('-q', '--queryfaa', type=str, required=True, help='Input query FASTA file')
    parser.add_argument('-m', '--modelscombined', type=str, required=True, help='Combined HMM model file')
    parser.add_argument('--hmmout', type=str, required=True, help='Output file for HMM hits')
    parser.add_argument('-hf', '--filtered_hmmout', type=str, required=True, help='Output file for HMM hits filtered')
    parser.add_argument('-c', '--countout', type=str, required=True, help='Output file for marker gene counts')
    parser.add_argument('-s', '--scoreout', type=str, required=True, help='Output file for highest bitscore matrix')
    parser.add_argument('-f', '--cutoff_file', type=str, help='Cutoff file for filtering HMM hits')

    args = parser.parse_args()

    # Only create directory if path has a directory component
    if os.path.dirname(args.hmmout):
        os.makedirs(os.path.dirname(args.hmmout), exist_ok=True)
    # Define hmmsearch command
    hmmsearch_cmd = f"hmmsearch --noali --domtblout {args.hmmout} --cpu 4 {args.modelscombined} {args.queryfaa}"

    # Check if the hmmsearch output file already exists
    if not os.path.exists(args.hmmout) or os.path.getsize(args.hmmout) == 0:
        # Run hmmsearch command
        run_cmd(hmmsearch_cmd)
    else:
        print(f"Skipping hmmsearch as the output file {args.hmmout} already exists.")

    models = get_models(args.modelscombined)
    ...
    # Get model names
    count_matrix, score_matrix = process_hmmout(models, args.hmmout, args.filtered_hmmout, cutoffs)
    ...
    if count_matrix.empty or score_matrix.empty:
        ...
        empty_count_matrix.to_csv(args.countout, sep='\t')
        empty_score_matrix.to_csv(args.scoreout, sep='\t')
    else:
        # Save count and score matrices with 'genomeid' as the first column
        count_matrix.reset_index().rename(columns={'index': 'genomeid'}).to_csv(args.countout, sep='\t', index=False)
        score_matrix.reset_index().rename(columns={'index': 'genomeid'}).to_csv(args.scoreout, sep='\t', index=False)
