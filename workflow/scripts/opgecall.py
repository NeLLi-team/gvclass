...
def main(fnafile: str, gffout: str, genecalling_statsout: str, summary_statsout: str, finalfaa: str, modelscombined: str, finalfna: str, hmmout: str) -> None:
    ...
    # Gene calling
    codes = [0, 1, 4, 6, 15, 11, 29]
    # For now, don't try to use codes 106 and 129 until we have the forked version
    print("Using standard pyrodigal")
    for code in codes:
    ...
    if float(coding_dens_codemeta) > 0:
        ...
        # Check all codes that were actually run
        code_list = ["code1", "code4", "code6", "code11", "code15", "code29"]
        # Add custom codes if they were run
        if any("code106" in str(f) for f in faa_files):
            code_list.extend(["code106", "code129"])
