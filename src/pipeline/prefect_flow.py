...
def process_query_task(
    ...
    if query_file.suffix == '.fna':
        ...
        best_code, outputs, best_metrics, all_code_results = optimizer.optimize_genetic_code(
        ...
        with open(comparison_file, 'w') as f:
            f.write("ttable\tGenes\tBases\tCodingDensity\tcomplete_bestHits\tavg_bestHit_score\n")
            ...
            for result in sorted_results:
                ...
                f.write(f"{ttable}\t{result['genes']}\t{result['bases']}\t"
        ...
        with open(stats_tsv, 'w') as f:
            ...
            f.write(f"0\t{estimated_genome_length}\t0.0\t{protein_count}\t0.0\tno_fna\n")
    ...
    if marker_files:
        ...
        logger.info(f"Processing {len(marker_files)} markers in parallel with {threads} threads")
        ...
        with ThreadPoolExecutor(max_workers=min(threads, len(marker_files))) as executor:
            # Submit all marker processing tasks
            futures = [executor.submit(process_single_marker, marker_data) 
    ...
    with open(individual_summary, 'w') as f:
        ...
        headers = ['query', 'taxonomy_majority', 'taxonomy_strict', 'species', 'genus', 
                  'family', 'order', 'class', 'phylum', 'domain', 'avgdist', 
                  'order_dup', 'order_completeness', 'gvog4_unique', 'gvog8_unique', 
                  'gvog8_total', 'gvog8_dup', 'mcp_total', 'mirus_unique', 'mirus_total', 
                  'mirus_dup', 'mrya_unique', 'mrya_total', 'phage_unique', 'phage_total', 
                  'cellular_unique', 'cellular_total', 'cellular_dup', 'contigs', 'LENbp', 
                  'GCperc', 'genecount', 'CODINGperc', 'ttable']
...
def create_final_summary_task(
    ...
    columns = [
        ...
        'avgdist', 'order_dup', 'order_completeness',
        'gvog4_unique', 'gvog8_unique', 'gvog8_total', 'gvog8_dup',
        'mcp_total', 'mirus_unique', 'mirus_total', 'mirus_dup',
        'mrya_unique', 'mrya_total', 'phage_unique', 'phage_total',
            ...
            if result['status'] == 'complete' and 'summary_data' in result:
                ...
                for col in columns:
                    ...
                    if isinstance(value, float):
                        if col in ['avgdist', 'order_dup', 'gvog8_dup', 'mirus_dup', 'cellular_dup']:
                            value = f"{value:.2f}"
...
def gvclass_flow(
    ...
    if threads_per_worker is None:
        ...
        n_workers = min(
            n_queries,
            total_threads // threads_per_worker,
            max_workers or n_queries
        )
    
    logger.info(f"Parallelization strategy: {n_workers} workers × {threads_per_worker} threads")
    ...
    if cluster_type == "local":
        ...
        logger.warning(f"Unsupported cluster type: {cluster_type}, using concurrent execution")
