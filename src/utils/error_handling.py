...
class GVClassError(Exception):
    ...
    def __init__(self, message: str, error_code: str = "GVCLASS_ERROR", context: Optional[Dict[str, Any]] = None):
...
class ProcessingError(GVClassError):
    ...
    def __init__(self, message: str, step: Optional[str] = None, input_file: Optional[str] = None):
        context = {}
        if step:
            context['step'] = step
        if input_file:
...
def validate_input(value: Any, field: str, validator: Callable[[Any], bool], error_msg: str) -> Any:
    """
