...
class TreeAnalyzer:
    ...
    def get_neighbors_from_tree(self, tree_file: Path, query_id: str) -> Dict[str, Dict[str, float]]:
        """
        ...
        try:
            ...
            for query_node in query_nodes:
                nearest, distance = self._find_nearest_neighbor(tree, query_node, query_id)
                if nearest:
    ...
    def _find_nearest_neighbor(self, tree: Tree, query_node: Tree, query_id: str) -> Tuple[Optional[str], float]:
        """
            ...
            for query_protein, neighbors in query_neighbors.items():
                for neighbor, distance in neighbors.items():
                    ...
                    if genome_id in self.labels_dict:
                        ...
                        tax_idx_map = {'kingdom': 0, 'order': 2, 'family': 3, 'genus': 4}
                ...
                for marker, query_neighbors in all_neighbors.items():
                    for query_protein, neighbors in query_neighbors.items():
                        for neighbor, distance in neighbors.items():
                            ...
                            f.write(f"{marker}\t{query_protein}\t{neighbor}\t{distance:.6f}\t{tax_str}\n")
            logger.info("Successfully wrote tree_nn file")
        else:
            logger.warning("No neighbors found for any markers - not writing tree_nn file")
