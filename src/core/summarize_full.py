...
from src.config.marker_sets import (
    MIRUS_MODELS, BUSCO_MODELS, PHAGE_MODELS,
    GVOG4M_MODELS, GVOG8M_MODELS, UNI56_MODELS,
    MCP_MODELS, MRYA_MODELS
)
...
class FullSummarizer:
    ...
    def calculate_order_metrics(self, counts_file: Path, order_tax: str) -> Tuple[float, float]:
    ...
    def summarize_query_full(
        ...
        result = {
            ...
            'ttable': basic_stats.get('ttable', 'unknown') if basic_stats.get('ttable', 'unknown') != '0' else 'codemeta'
        }
                        ...
                        for level, idx in tax_level_mapping.items():
                            if idx < len(tax_info):
                                ...
                                if level == 'domain':
                                    ...
                                    tax_counters[level][f"{genome_id.split('__')[0]}__{tax_value}"] += 1
        ...
        if counts_file.exists():
            ...
            for metric in ['gvog4_unique', 'gvog8_unique', 'gvog8_total', 'mcp_total',
                          'mirus_unique', 'mirus_total', 'mrya_unique', 'mrya_total',
                          'phage_unique', 'phage_total', 'cellular_unique', 'cellular_total']:
                result[metric] = 0
        ...
        if order_tax:
            completeness, duplication = self.calculate_order_metrics(counts_file, order_tax)
