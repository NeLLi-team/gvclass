...
class EnhancedCPUMonitor:
    ...
    def __init__(self, output_dir, interval=0.5, target_process=None):
        ...
        # Enhanced tracking
        self.samples = []
        self.process_history = defaultdict(
            lambda: {
                "samples": [],
                "cmdline": "",
                "name": "",
    ...
    def monitor(self):
        ...
        while self.running:
            ...
            cpu_freq = (
                psutil.cpu_freq(percpu=True) if hasattr(psutil, "cpu_freq") else None
            )
            ...
            if self.target_process:
                try:
                    ...
                    for proc in all_procs:
                        ...
                        if proc_info and (
                            proc_info["cpu_percent"] > 0 or proc_info["memory_mb"] > 10
                        ):
                    ...
                    if any(
                        ...
                        if proc.pid not in process_tree:
                            ...
                            if proc_info and (
                                ...
                                self.process_history[pid]["cmdline"] = proc_info[
                                    "cmdline"
                                ]
                                self.process_history[pid]["name"] = proc_info["name"]
                                self.process_history[pid]["parent_pid"] = proc_info[
                                    "parent_pid"
                                ]
            ...
            sample = {
                ...
                "swap": {"used_mb": swap.used / 1024 / 1024, "percent": swap.percent},
                "io": {
                    "read_mb": (
            ...
            if int(elapsed) % 5 == 0:
                print(
                    f"\r[{elapsed:6.0f}s] CPU: {cpu_overall:5.1f}% ({active_cores} cores active) | "
    ...
    def create_enhanced_summary(self):
        ...
        with open(self.summary_file, "w") as f:
            ...
            f.write(
                f"Pipeline Runtime: {duration:.1f} seconds ({duration/60:.1f} minutes)\n"
            )
            f.write(
                f"Monitoring Samples: {len(self.samples)} (interval: {self.interval}s)\n"
            )
            f.write(
                f"CPU Cores: {len(self.samples[0]['cpu']['per_core'])} logical cores\n\n"
            )
            ...
            f.write(
                f"Time > 50%: {sum(1 for v in cpu_values if v > 50)/len(cpu_values)*100:.1f}%\n"
            )
            f.write(
                f"Time > 75%: {sum(1 for v in cpu_values if v > 75)/len(cpu_values)*100:.1f}%\n"
            )
            f.write(
                ...
                core_stats.append(
                    {
                        ...
                        "max": max(core_values),
                        "active_time": sum(1 for v in core_values if v > 25)
                        / len(core_values)
                        * 100,
                    }
                )
            ...
            for stat in sorted(core_stats, key=lambda x: x["avg"], reverse=True):
                f.write(
                    f"Core {stat['core']:2d}: Avg={stat['avg']:5.1f}% Max={stat['max']:5.1f}% "
            ...
            for phase, times in self.phase_detection.items():
                if times:
                    ...
                    f.write(
                        f"{phase:20s}: {phase_duration:6.1f}s "
            ...
            process_types = defaultdict(
                lambda: {"count": 0, "total_cpu": 0, "total_time": 0}
            )
            ...
            for pid, data in self.process_history.items():
                if data["samples"]:
                    ...
                    process_types[process_type]["total_cpu"] += sum(
                        s["cpu"] for s in data["samples"]
                    )
                    process_types[process_type]["total_time"] += (
                        data["end_time"] - data["start_time"]
                    )

            f.write("\nProcess Type Distribution:\n")
            for ptype, stats in sorted(
                process_types.items(), key=lambda x: x[1]["total_cpu"], reverse=True
            ):
                avg_cpu = stats["total_cpu"] / max(stats["count"], 1)
                f.write(
                    f"{ptype:20s}: {stats['count']:3d} instances, "
            ...
            for sample in self.samples:
                ...
                if len(active_procs) > 1:
                    ...
                    efficiency = (
                        ...
                        / (sample["cpu"]["active_cores"] * 100)
                        if sample["cpu"]["active_cores"] > 0
                        else 0
                    )
            ...
            f.write(
                f"Average CPU efficiency during parallel phases: {avg_efficiency:.1f}%\n"
            )
            f.write(
                f"Peak parallel processes: {max(s['process_summary']['active'] for s in self.samples)}\n"
            )
                ...
                if sample["cpu"]["overall_percent"] < 25:
                    if current_period is None:
                        current_period = {
                            "start": i,
                            "end": i,
                            "min_cpu": sample["cpu"]["overall_percent"],
                        }
                    else:
                        current_period["end"] = i
                        current_period["min_cpu"] = min(
                            current_period["min_cpu"], sample["cpu"]["overall_percent"]
                        )
                else:
                    if (
                        current_period
                        and (current_period["end"] - current_period["start"]) > 5
                    ):
            ...
            if low_cpu_periods:
                f.write(
                    f"Detected {len(low_cpu_periods)} low CPU utilization periods:\n"
                )
            ...
            if peak_memory_procs:
                ...
                for proc in sorted(
                    peak_memory_procs, key=lambda x: x["peak_mb"], reverse=True
                )[:10]:
    ...
    def create_timeline(self):
        ...
        with open(self.timeline_file, "w") as f:
            ...
            # Draw timeline
            f.write(
                "Time (s)  CPU Usage                                          Active Phases\n"
            )
            f.write("-" * 80 + "\n")
            ...
            for i in range(n_buckets):
                if cpu_buckets[i]:
                    ...
                    f.write(
                        f"{i*bucket_size:4d}-{(i+1)*bucket_size:3d}  "
...
def main():
    ...
    parser.add_argument(
        "-o",
        "--output-dir",
        default="cpu_monitoring_v2",
        help="Output directory for logs",
    )
    parser.add_argument(
        "-i", "--interval", type=float, default=0.5, help="Sampling interval in seconds"
    )
