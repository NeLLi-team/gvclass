...
if command -v docker &> /dev/null; then
    ...
    if [[ "$response" =~ ^([yY][eE][sS]|[yY])$ ]]; then
        ...
        if docker build -t gvclass:1.6.0 -f containers/docker/Dockerfile .; then
            docker tag gvclass:1.6.0 gvclass:latest
...
echo -e "  Docker:     docker run -v /path/to/data:/data gvclass:1.6.0 /data -t 32"
