...
docker build -t gvclass:2.0.2 -t gvclass:latest -f containers/docker/Dockerfile .

# Export Docker image for Apptainer conversion
echo "Exporting Docker image..."
docker save gvclass:2.0.2 -o gvclass_docker.tar
...
if command -v apptainer &> /dev/null; then
    ...
    echo "Docker image built successfully: gvclass:2.0.2"
fi
...
echo "Usage examples:"
echo "Docker:"
echo "  docker run -v /path/to/data:/data -v /path/to/results:/results -v /path/to/cache:/resource-cache gvclass:2.0.2 /data -o /results"
echo ""
