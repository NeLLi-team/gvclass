...
On first use the wrapper pulls `library://nelligroup-jgi/gvclass/gvclass:2.0.2` from the public Sylabs library and caches it under `~/.cache/gvclass/images/`. Later runs reuse the cached image. The image carries the v2.0.0 database and all tools, so you skip database setup entirely.
...
apptainer pull --library https://library.sylabs.io \
  gvclass_2.0.2.sif library://nelligroup-jgi/gvclass/gvclass:2.0.2
...
apptainer run -B /path/to/bins:/input -B /path/to/results:/output \
  ...
  gvclass_2.0.2.sif /input -o /output -t 32
```
