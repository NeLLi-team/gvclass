...
=== "Apptainer (HPC)"
    ...
    The wrapper pulls the `library://nelligroup-jgi/gvclass/gvclass:2.0.2` Apptainer image and keeps the compact-resource warm cache under `~/.cache/gvclass/resource-cache/v2.0.0`.

For the full walkthrough, see [Getting started](tutorials/getting-started.md).

!!! note "Current release"

    Software 2.0.2. `pixi run setup-db` downloads the public v2.0.0 resource archive from [Zenodo](https://doi.org/10.5281/zenodo.21225457) and installs database `DB_VERSION` v2.0.0. Downloads and changelogs are on the [GitHub Releases](https://github.com/NeLLi-team/gvclass/releases) page.
