...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-dir", default="benchmarking/contamination/contamination_benchmark_faa"
    )
    parser.add_argument(
        "--metadata",
    ...
    parser.add_argument(
        "--query-results",
        default="benchmarking/completeness/refs-Feb-2026-fna-pox10-genus10/gvclass_extended_results",
    )
    ...
    parser.add_argument(
        "--export-model",
        nargs="?",
        const="resources/contamination/model.joblib",
...
def extract_query_faa(query_results_dir: Path, accession: str) -> List[SeqRecord]:
    ...
    with tarfile.open(tar_path, "r:gz") as tf:
        member = next(
            m for m in tf.getmembers() if m.name.endswith(f"query_faa/{accession}.faa")
        )
...
def choose_donor(
    base: Dict[str, Any], rows: List[Dict[str, Any]], related: bool
) -> Dict[str, Any]:
    if related:
        pool = [
            r
            for r in rows
            if r["family"] == base["family"]
            and r["accession_number"] != base["accession_number"]
            and 100_000 <= r["genome_size_bp"] <= 700_000
            and r["num_contigs"] == 1
        ]
        if pool:
        ...
        pool = [
            r
            for r in rows
            if r["order_name"] == base["order_name"]
            and r["accession_number"] != base["accession_number"]
            and 100_000 <= r["genome_size_bp"] <= 700_000
            and r["num_contigs"] == 1
        ]
...
def build_cellular_protein_pool(database: Path) -> List[SeqRecord]:
    ...
    for marker in BUSCO_MODELS[:40] + UNI56_MODELS[:20]:
        ...
        for record in SeqIO.parse(str(path), "fasta"):
            if (
                record.id.startswith("BAC__")
                or record.id.startswith("ARC__")
                or record.id.startswith("EUK__")
            ):
                pool.append(record)
        if len(pool) >= 500:
            break
...
def select_proteins_to_fraction(
    records: List[SeqRecord], target_fraction: float, base_bp: int, rng: random.Random
) -> List[SeqRecord]:
    target_bp = max(
        900, round((base_bp * target_fraction) / max(1e-9, 1.0 - target_fraction))
    )
    shuffled = records[:]
...
def rename_records(
    records: List[SeqRecord], contig_id: str, prefix: str
) -> List[SeqRecord]:
    renamed = []
...
def write_synthetic_faa(
    ...
    records = [
        SeqRecord(record.seq, id=record.id, description="") for record in base_records
    ]
    ...
    if scenario.startswith("hgt_cellular"):
        ...
        injected = rename_records(
            donor_records, main_contig, base_row["accession_number"]
        )
        ...
        donor_records = select_proteins_to_fraction(
            cellular_pool, target_fraction, base_bp, rng
        )
        contig_id = f"cellularcontam_{scenario}"
        injected = rename_records(
            donor_records, contig_id, base_row["accession_number"]
        )
        records.extend(injected)
        added_bp = sum(protein_bp(r) for r in donor_records)
        ...
        contamination_type = (
            "viral_contig_related"
            if "related" in scenario
            else "viral_contig_divergent"
        )
        donor_row = choose_donor(base_row, all_rows, related="related" in scenario)
        donor_records = extract_query_faa(
            query_results_dir, donor_row["accession_number"]
        )
        donor_contig = largest_contig_id(donor_records)
        donor_records = [r for r in donor_records if protein_contig(r) == donor_contig]
        donor_records = select_proteins_to_fraction(
            donor_records, target_fraction, base_bp, rng
        )
...
def generate_dataset(
    ...
    for family in BASE_FAMILIES:
        for base_row in by_family[family]:
            base_records = extract_query_faa(
                query_results_dir, base_row["accession_number"]
            )
...
def train_and_benchmark(feature_df: pd.DataFrame, run_dir: Path) -> Dict[str, Any]:
    ...
    model_name, model = choose_best_model(
        train_df[METHOD3_FEATURES].fillna(0.0),
        train_df["true_contamination_pct"].astype(float),
        train_df["base_accession"],
    )
    ...
    contamination_type_means = (
        test_df.groupby("contamination_type")[
            [
                "contamination_score_v1",
                "suspicious_bp_fraction_v2",
                "method3_score",
    ...
    report_lines = [
        ...
        as_tsv_block(
            ranked[
                ["method", "auroc", "aupr", "f1", "mae_all", "hgt_false_positive_rate"]
            ]
        ),
        "```",
        "",
...
def main() -> None:
    ...
    manifest_path = generate_dataset(
        run_dir, metadata_rows, query_results_dir, database, args.force_regenerate
    )
