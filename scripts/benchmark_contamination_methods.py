...
from scripts._contamination_features import compute_query_features
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-dir", default="benchmarking/contamination/contamination_benchmark"
    )
    parser.add_argument(
        "--input-fna-dir",
        default="benchmarking/completeness/refs-Feb-2026-fna-pox10-genus10/fna",
    )
    parser.add_argument(
        "--metadata",
        default="benchmarking/completeness/refs-Feb-2026-fna-pox10-genus10/metadata_table.tsv",
    )
...
def select_base_genomes(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if len(selected) != len(BASE_FAMILIES) * 2:
        raise RuntimeError(
            f"Expected {len(BASE_FAMILIES) * 2} base genomes, found {len(selected)}"
        )
    return selected
...
def contamination_bp(base_bp: int, target_fraction: float) -> int:
    return max(
        2000, round((base_bp * target_fraction) / max(1e-9, 1.0 - target_fraction))
    )


def pick_related_donor(
    base: Dict[str, Any], all_rows: List[Dict[str, Any]]
) -> Dict[str, Any]:
    family_pool = [
        row
        for row in all_rows
        if row["family"] == base["family"]
        and row["accession_number"] != base["accession_number"]
        and row["num_contigs"] == 1
        and row["genome_size_bp"] >= 100_000
    ]
    if family_pool:
        return sorted(family_pool, key=lambda row: row["genome_size_bp"], reverse=True)[
            0
        ]
    order_pool = [
        row
        for row in all_rows
        if row["order_name"] == base["order_name"]
        and row["accession_number"] != base["accession_number"]
        and row["num_contigs"] == 1
        and row["genome_size_bp"] >= 100_000
    ]
...
def build_cellular_gene_pool() -> List[str]:
    gene_path = Path(
        ".pixi/envs/default/lib/python3.11/site-packages/pyhmmer/tests/data/seqs/CP040672.1.genes_100.fna"
    )
    return [
        str(record.seq)
        for record in SeqIO.parse(str(gene_path), "fasta")
...
def build_cellular_contig_source() -> str:
    contig_path = Path(
        ".pixi/envs/default/lib/python3.11/site-packages/pyhmmer/tests/data/seqs/1390.SAMEA104415756.OFHT01000022.fna"
    )
...
def write_synthetic_bin(
    ...
    if scenario.startswith("hgt_cellular"):
        ...
        pieces = [
            slice_sequence(gene, min(len(gene), 2000), rng)
            for gene in rng.sample(cellular_genes, n_genes)
        ]
        ...
        records.append(
            SeqRecord(Seq(seq), id=f"cellular_contam_{scenario}", description="")
        )
        ...
        contamination_type = (
            "viral_contig_divergent"
            if "divergent" in scenario
            else "viral_contig_related"
        )
        donor_row = (
            pick_divergent_donor(base_row, all_rows)
            if "divergent" in scenario
            else pick_related_donor(base_row, all_rows)
        )
        donor_records = load_fna_records(
            input_fna_dir / f"{donor_row['accession_number']}.fna"
        )
        ...
        records.append(
            SeqRecord(Seq(seq), id=f"viral_contam_{scenario}", description="")
        )
        truth_fraction = (len(seq) / (base_bp + len(seq))) * 100.0
    else:
        contamination_type = "clean"
...
def generate_dataset(
    run_dir: Path, input_fna_dir: Path, metadata_rows: List[Dict[str, Any]], force: bool
) -> Path:
    ...
    for family in BASE_FAMILIES:
        for base_row in base_rows_by_family[family]:
            base_records = load_fna_records(
                input_fna_dir / f"{base_row['accession_number']}.fna"
            )
    ...
    with manifest_path.open("w", newline="") as handle:
        writer = csv.DictWriter(
            handle, fieldnames=list(manifest_rows[0].keys()), delimiter="\t"
        )
        writer.writeheader()
...
def run_gvclass(inputs_dir: Path, results_dir: Path, workers: int, force: bool) -> None:
    if (
        results_dir.exists()
        and (results_dir / "gvclass_summary.tsv").exists()
        and not force
    ):
        return
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="gvclass_contam_") as tmpdir:
                ...
                with tarfile.open(tar_path, "r:gz") as tf:
                    members = [
                        m for m in tf.getmembers() if any(s in m.name for s in wanted)
                    ]
...
def choose_best_model(X: pd.DataFrame, y: pd.Series, groups: pd.Series) -> Any:
    candidates = [
        (
            "random_forest",
            RandomForestRegressor(
                n_estimators=400, random_state=RANDOM_SEED, min_samples_leaf=2
            ),
        ),
        (
            "extra_trees",
            ExtraTreesRegressor(
                n_estimators=500, random_state=RANDOM_SEED, min_samples_leaf=2
            ),
        ),
        (
            "hist_gbm",
            HistGradientBoostingRegressor(
                random_state=RANDOM_SEED, max_depth=6, min_samples_leaf=5
            ),
        ),
    ]
...
def evaluate_predictions(
    ...
    mae_contaminated = (
        mean_absolute_error(
            contaminated["true_contamination_pct"], contaminated[score_col]
        )
        if not contaminated.empty
        else 0.0
    )
    hgt = df[df["contamination_type"] == "hgt_like"]
    hgt_false_positive_rate = (
        float((hgt[score_col] >= threshold).mean()) if not hgt.empty else 0.0
    )
    return {
...
def build_feature_table(
    manifest_path: Path, results_dir: Path, database: Path
) -> pd.DataFrame:
...
def train_and_benchmark(feature_df: pd.DataFrame, run_dir: Path) -> Dict[str, Any]:
    ...
    method1_threshold = optimize_threshold(
        train_df["contamination_present"].astype(int).tolist(),
        train_df["contamination_score_v1"].astype(float).tolist(),
    )
    method2_threshold = optimize_threshold(
        train_df["contamination_present"].astype(int).tolist(),
        train_df["suspicious_bp_fraction_v2"].astype(float).tolist(),
    )
    ...
    method3_threshold = optimize_threshold(
        train_df["contamination_present"].astype(int).tolist(),
        train_df["method3_score"].astype(float).tolist(),
    )
    ...
    feature_df_out.to_csv(
        run_dir / "benchmark_feature_table.tsv", sep="\t", index=False
    )
    ...
    report_lines = [
        ...
        "",
        test_df.groupby("scenario")[
            [
                "contamination_score_v1",
                "suspicious_bp_fraction_v2",
                "method3_score",
                "true_contamination_pct",
            ]
        ]
        .mean()
        .round(2)
        ...
        "## HGT False Positives",
        "",
        test_df[test_df["contamination_type"] == "hgt_like"][
            [
                "sample_id",
                "contamination_score_v1",
                "suspicious_bp_fraction_v2",
    ...
    (run_dir / "method3_model_meta.json").write_text(
...
def main() -> None:
    ...
    manifest_path = generate_dataset(
        run_dir, input_fna_dir, metadata_rows, args.force_regenerate
    )
    run_gvclass(run_dir / "inputs", run_dir / "results", args.workers, args.force_rerun)
    feature_df = build_feature_table(
        manifest_path, run_dir / "results", Path(args.database).resolve()
    )
    ...
    for _, row in outcome["metrics_df"].iterrows():
        print(
            f"{row['method']}\tAUROC={row['auroc']}\tAUPR={row['aupr']}\tMAE={row['mae_all']}\tF1={row['f1']}"
        )
